...
import com.lbw.blackcat.utils.GsonUtil;
...
public class QnUploadHelper {
    ...
    private static QnUploadHelper qnUploadHelper;

    public static QnUploadHelper getInstance(){
        if (qnUploadHelper == null) {
            synchronized (QnUploadHelper.class){
                if (qnUploadHelper == null) {
                    qnUploadHelper = new QnUploadHelper();
                }
            }
        }
        return qnUploadHelper;
    }


    public void init(String accessKey, String secretKey, String domain, String bucketName) {
        AccessKey = accessKey;
        SecretKey = secretKey;
        Domain = domain;
     ...
     */
    public void uploadPic(final String path, final String keys, final UploadCallBack callBack) {
        try {
            // 1:第一种方式 构造上传策略
//            JSONObject _json = new JSONObject();
//            _json.put("deadline", delayTimes);
//            _json.put("scope", BucketName);
            JsonObject json = GsonUtil.BeanToJsonObject(new ReturnBody(delayTimes,BucketName));
